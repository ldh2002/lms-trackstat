...
sub init {
	...
	if($driver eq 'mysql') {
		...
		$useLongUrls = 1;
		if(defined($majorMysqlVersion) && ($majorMysqlVersion<5 || !$prefs->get("long_urls"))) {
			$useLongUrls = 0;
	...
	if($driver eq 'mysql') {
		...
		Plugins::TrackStat::Storage::executeSQLFile("dbindex.sql");
	}
	...
	if($prefs->get('musicbrainz_enabled')) {
		...
		if($driver eq 'mysql') {
			...
			if( $@ ) {
				...
				# Update only statistics records with invalid url
				$sql = "UPDATE track_statistics SET url=(select url from temp_track_statistics where musicbrainz_id=track_statistics.musicbrainz_id),urlmd5=(select urlmd5 from temp_track_statistics where musicbrainz_id=track_statistics.musicbrainz_id) where exists (select url from temp_track_statistics where musicbrainz_id=track_statistics.musicbrainz_id) and not exists (select track_statistics.musicbrainz_id, track_statistics.urlmd5 from track_statistics inner join temp_track_statistics on track_statistics.musicbrainz_id = temp_track_statistics.musicbrainz_id and track_statistics.urlmd5 = temp_track_statistics.urlmd5)";
				$sth = $dbh->prepare( $sql );
				$count = 0;
				eval {
					$count = $sth->execute();
	...
	if($prefs->get("history_enabled")) {
		...
		if($prefs->get('musicbrainz_enabled')) {
			...
			if($driver eq 'mysql') {
					...
					# Update only history records with invalid URL 
					$sql = "UPDATE track_history SET url=(select tracks.url from tracks where tracks.musicbrainz_id is not null and tracks.musicbrainz_id=track_history.musicbrainz_id and track_history.urlmd5!=tracks.urlmd5 and length(tracks.url)<".($useLongUrls?512:256)."),urlmd5=(select tracks.urlmd5 from tracks where tracks.musicbrainz_id is not null and tracks.musicbrainz_id=track_history.musicbrainz_id and track_history.urlmd5!=tracks.urlmd5 and length(tracks.url)<".($useLongUrls?512:256).") where exists (select tracks.url from tracks where tracks.musicbrainz_id is not null and tracks.musicbrainz_id=track_history.musicbrainz_id and track_history.urlmd5!=tracks.urlmd5 and length(tracks.url)<".($useLongUrls?512:256).") and not exists (select track_history.musicbrainz_id, track_history.urlmd5 from track_history inner join tracks on track_history.musicbrainz_id = tracks.musicbrainz_id and track_history.urlmd5 = tracks.urlmd5)";
			}
			$sth = $dbh->prepare( $sql );
			$count = 0;
			eval {
